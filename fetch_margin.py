...
def main():
    date_compact = today_str_yyyymmdd()
    date_dash = today_str_dash()

    df = fetch_margin(date_compact)
    if df is None:
        print("今天抓不到融資融券資料，可能是非交易日，程式正常結束。")
        return

    os.makedirs("data/margin", exist_ok=True)
    out_path = f"data/margin/{date_dash}.csv"
    df.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"[融資融券] 已存檔：{out_path}（{len(df)} 檔股票）")
